...
class BonitoSLSTM(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, reverse = False):
        ...
        self.rnn = snn.SLSTM(in_channels, out_channels, num_layers = 1, bidirectional = False, bias = True)
        self.reverse = reverse
...
try:
    ...
    class CTC_CRF(SequenceDist):
        ...
        def logZ(self, scores, S:semiring=Log):
            ...
            return logZ_cu_sparse(Ms, self.idx, alpha_0, beta_T, S)  #seqdist.sparse.logZ
        ...
        def forward_scores(self, scores, S: semiring=Log):
            ...
            alpha_0 = Ms.new_full((N, self.n_base**(self.state_len)), S.one)
            return fwd_scores_cu_sparse(Ms, self.idx, alpha_0, S, K=1)   #seqdist.sparse.fwd_scores_cupy

        def backward_scores(self, scores, S: semiring=Log):
            T, N, _ = scores.shape
            Ms = scores.reshape(T, N, -1, self.n_base + 1)
            beta_T = Ms.new_full((N, self.n_base**(self.state_len)), S.one)
            return bwd_scores_cu_sparse(Ms, self.idx, beta_T, S, K=1) #seqdist.sparse.bwd_scores_cupy
